"""Run one capped Base mainnet x402 payment against SmartFetch.

The buyer private key is requested interactively and is used only to create an
in-memory signer. This script accepts no command-line arguments, reads no key
from the environment, and writes no files.
"""
...
def build_x402_client(account: LocalAccount) -> x402ClientSync:
    """Build an exact-payment client restricted to Base mainnet."""
    signer = EthAccountSigner(account)
    client = x402ClientSync()
    client.register(BASE_MAINNET, ExactEvmClientScheme(signer))
...
def _print_result(response: object, buyer_address: str) -> bool:
    ...
    print(f'Network: {settlement.network if settlement else BASE_MAINNET}')
    print(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print('Paid fetch failed.', file=sys.stderr)
        return 1
    finally:
